...
local ZonesServiceClient    = safeRequire(Features.Zones.ZonesServiceClient)
...
local activeCodes = {
    "giveMeLuckNOW","outofthisworld","sliming","AATOMORROW","goingBananas",
    "test", 
    "gullible",
    "fruitfeast",
}
...
task.spawn(function()
    while task.wait(1) do
        if not Config.AutoCollectRecipe then continue end
        pcall(function()
            ...
            for _, zoneModel in ipairs(zonesFolder:GetChildren()) do
                if not Config.AutoCollectRecipe then break end

                local recipe = zoneModel:FindFirstChild("Recipe")
                if not recipe then continue end

                local attachment = recipe:FindFirstChild("RecipePromptAttachment", true)
                if not attachment then continue end

                local prompt = attachment:FindFirstChildWhichIsA("ProximityPrompt")
                if not prompt then continue end

                local recipePart = recipe:IsA("BasePart") and recipe
                    or recipe:FindFirstChildWhichIsA("BasePart", true)

                if recipePart then
                    hrp.CFrame = recipePart.CFrame + Vector3.new(0, 4, 0)
                    task.wait(0.15)
                end
    ...
    while task.wait(0.1) do
        if Config.AutoFarmBestMob then
            pcall(function()
                ...
                if lastTeleportedZone ~= bestZoneNum then
                    if ZonesServiceClient and type(ZonesServiceClient.teleportToZone) == "function" then
                        ZonesServiceClient:teleportToZone(bestZoneNum)
                        lastTeleportedZone = bestZoneNum
                        task.wait(1)
                    end
                end

                local targetZoneFolder = Workspace:FindFirstChild("Gameplay" .. tostring(bestZoneNum))
                if not targetZoneFolder then
                    local minZoneDist = math.huge
                    for _, child in ipairs(Workspace:GetChildren()) do
                        if child.Name:sub(1, 8) == "Gameplay" then
                            local part = child:FindFirstChildWhichIsA("BasePart", true)
                            if part then
                                local d = (hrp.Position - part.Position).Magnitude
                                if d < minZoneDist then
                                    minZoneDist = d
                                    targetZoneFolder = child
                                end
                            end
                        end
                    end
                ...
                local enemiesFolder = targetZoneFolder:FindFirstChild("Enemies")
                ...
                local gpRemotes = Remotes:FindFirstChild(targetZoneFolder.Name)
                if gpRemotes then
                    local remoteEvent = gpRemotes:FindFirstChild("RemoteEvent")
                    if remoteEvent then
                        local equippedSlimes = getEquippedSlimes(targetZoneFolder)
                        for _, enemy in ipairs(enemies) do
...
_UI.AddToggle(zoneGroup, "AutoCollectRecipe", {
    Title = "Auto Collect Recipe",
    Description = "Automatically collects Recipe from all zones",
    Default = Config.AutoCollectRecipe,
    Callback = function(val) Config.AutoCollectRecipe = val end
})
